...
function Del_page() {
  ...
  const remove = () => {
    ...
    const ques_click_list_nqna = ques
      .filter((obj) => obj.clicked === true)
      .map((obj) => obj.nQnA_id);
    {
      ques_click_list_nqna.map((nqna_id) => Delquestion(nqna_id));
    }

    // 프론트 redux 상에서 질문 삭제

    const ques_click_list = ques
      .filter((obj) => obj.clicked === true)
      .map((obj) => obj.id);
    {
      ques_click_list.map((id) => dispatch(questionSlice.actions.remove(id)));
    }

    // 백엔드 db 상에서 답변 삭제

    const ans_click_list_nqna = ans
      .filter((obj) => obj.clicked === true)
      .map((obj) => obj.nQnA_id);
    {
      ans_click_list_nqna.map((nqna_id) => Delanswer(nqna_id));
    }

    // 프론트 redux 상에서 답변 삭제

    const ans_click_list = ans
      .filter((obj) => obj.clicked === true)
      .map((obj) => obj.id);
    {
      ans_click_list.map((id) => dispatch(AnswerSlice.actions.remove(id)));
    }
  ...
  return (
    <div className={styles.background}>
      ...
      <div className={styles.remover} onClick={remove}>
        <p
          className={`${styles.remover_p} ${allChosen && styles.remover_p_red}`}
        >
          삭제
        </p>
      </div>
