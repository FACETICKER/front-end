...
const EditQues_Modal = () => {
  ...
  const nqna_id = ques
    ...
    .map((obj) => obj.nQnA_id)[0]; // 클릭한 질문 nqna아이디

  const first_ques_open = ques
    .filter((obj) => obj.clicked === true)
    .map((obj) => obj.open)[0]; // 초기 질문 비공개값
  ...
  const QuesHiddenPatch = () => {
    ...
    fetch(
      `http://app.faceticker.site/${userID}/nqna/${nqna_id}/question/hidden`,
      {
